...
final class AudioPlaybackDeck {
    let player: AVPlayer
    let autoMixEqualizerState = SharedAutoMixEqualizerState()
    ...
    private(set) var mediaTimeline = AudioPlaybackMediaTimeline()
        ...
        currentItemObserver = player.observe(
            ...
            Task { @MainActor [self, item] in
                ...
                guard let precisePlayer = self.player as? MeloXAudioPlayer,
                      let timeline = precisePlayer.preciseTimeline(for: item)
                else { return }
                self.mediaTimeline = timeline
                self.installObservers(for: item, preserveEqualizerState: true)
            }
        }
    }
    ...
    func replaceCurrentItem(
        ...
        installObservers(for: playbackItem.item, preserveEqualizerState: false)
        player.replaceCurrentItem(with: playbackItem.item)
        (player as? MeloXAudioPlayer)?.preparePreciseIfNeeded(for: playbackItem.item)
    }
    ...
    var currentPlaybackTime: TimeInterval? {
        ...
        return mediaTimeline.playbackPosition(forMediaTime: player.currentTime())
    }

    var playbackDuration: TimeInterval? {
        guard let item = player.currentItem else { return nil }
        return mediaTimeline.playbackDuration(forMediaDuration: item.duration)
    }

    func mediaTime(forPlaybackPosition position: TimeInterval) -> CMTime {
        mediaTimeline.mediaTime(forPlaybackPosition: position)
    }
